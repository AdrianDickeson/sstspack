...
def test_get_frequency_domain_seasonal_model_design():
    H = 2
    sigma2_omega = [3, 4]
    s = 4
...
def test_frequency_domain_model_terms():
    i = 2
    s = 4
    omega = 2 * PI / s
    sigma2 = 4
...
def test_get_ARMA_model_design():
    phi_terms = [0.25, -0.25]
    theta_terms = [0.4]
    Q = full((1, 1), 0.5)
    data_df = md.get_ARMA_model_design(short_model_rows, phi_terms, theta_terms, Q)
...
def test_get_SARMA_model_design():
    s = 7
    PHI_terms = [0.25, -0.25]
    THETA_terms = [0.4]
    Q = full((1, 1), 0.5)

    data_df = md.get_SARMA_model_design(short_model_rows, s, PHI_terms, THETA_terms, Q)
...
def test_get_ARMA_x_SARMA_model_design():
    phi_terms = [0.25, -0.25]
    theta_terms = [0.4]
    s = 4
    PHI_terms = [0.8]
    ...
    data_df = md.get_ARMA_x_SARMA_model_design(
        short_model_rows, phi_terms, theta_terms, s, PHI_terms, THETA_terms, Q
    )
...
def test_get_ARIMA_model_design():
    phi_terms = [0.25, -0.25]
    theta_terms = [0.4]
    Q = full((1, 1), 0.5)
    d = 1
    data_df = md.get_ARIMA_model_design(short_model_rows, phi_terms, d, theta_terms, Q)
...
def test_get_SARIMA_model_design():
    s = 4
    PHI_terms = [0.8]
    D = 2
    THETA_terms = []
    ...
    data_df = md.get_SARIMA_model_design(
        short_model_rows, s, PHI_terms, D, THETA_terms, Q
    )
...
def test_get_ARIMA_x_SARIMA_model_design():
    phi_terms = [0.25, -0.25]
    d = 1
    theta_terms = [0.4]
    s = 4
...
def test_get_intervention_model_design():
    ...
    data_df = md.get_intervention_model_design(abc_model_index, "b", Q=ones((1, 1)))
    ...
    data_df = md.get_intervention_model_design(3, 1, Q=zeros((2, 2)), H=zeros((2, 2)))
...
def test_get_time_varying_regression_model_design():
    H = full((1, 1), 2)
    Q = ones((1, 1))
