...
from numpy import full, ones, zeros, identity, hstack, vstack, pi as PI, diag,\
    block
import pandas as pd
from scipy.linalg.special_matrices import block_diag
from pandas.tests.indexes.multi.conftest import idx

def get_local_level_model_data(length_index, Q, H):
    '''
    '''
    Z = ones((1,1))
    d = zeros((1,1))
...
def get_local_linear_trend_model_data(length_index, Q, H):
    '''
    '''
    Z = ones((1,2))
    Z[0,1] = 0
...
def get_frequency_domain_seasonal_model_data(length_index, s, Q_list, H):
    ...
    while i <= s/2:
        ...
        submodel = get_static_model_df(length_index, Z=Z, d=d_i, H=H, T=T, c=c, R=R, Q=Q)
...
def get_SARMA_model_data(series_length_index, s, PHI_terms, THETA_terms, Q):
    '''
    '''
    return get_ARMA_x_SARMA_model_data(series_length_index, [], [], s, PHI_terms, THETA_terms, Q)

def get_ARMA_x_SARMA_model_data(series_length_index, phi_terms, theta_terms,
                                s, PHI_terms, THETA_terms, Q):
    '''
    '''
    return get_ARIMA_x_SARIMA_model_data(series_length_index, phi_terms, 0, theta_terms,
                                         s, PHI_terms, 0, THETA_terms, Q)

def get_ARIMA_model_data(series_length_index, phi_terms, d, theta_terms, Q):
    '''
    '''
    return get_ARIMA_x_SARIMA_model_data(series_length_index, phi_terms, d, theta_terms,
                                         1, [], 0, [], Q)

def get_SARIMA_model_data(series_length_index, s, PHI_terms, D, THETA_terms, Q):
    '''
    '''
    return get_ARIMA_x_SARIMA_model_data(series_length_index, [], 0, [],
                                         s, PHI_terms, D, THETA_terms, Q)

def get_ARIMA_x_SARIMA_model_data(series_length_index, phi_terms, d, theta_terms,
                                  s, PHI_terms, D, THETA_terms, Q):
    '''
    '''
    ...
    Z, d, H, T, c, R, Q = process_terms(H, Z, d, Q, T, c, R)
    result = get_static_model_df(series_length_index, Z=Z, d=d_term, H=H, T=T, c=c, R=R, Q=Q)
    return result
...
def get_intervention_model_data(length_index, intervention_point, Q=0, H=0):
    ...
    try:
        ...
        prior_index = [value for idx, value in enumerate(length_index) if idx < intervention_idx]
        post_index = [value for idx, value in enumerate(length_index) if idx >= intervention_idx]
...
def get_time_varying_regression_model_data(length_index, regressors_df, Q, H):
    ...
    for idx in index:
        ...
        Z, d, H, T, c, R, Q = process_terms(H, Z, d_master, Q, T_master, c_master, R_master)
