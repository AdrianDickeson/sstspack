...
def plot_seasonal_breakdown(axs, model, level_indexes, seasonal_indexes):
    '''
    '''
    level_data = array([dot(model.Z[idx][:,level_indexes], model.a_hat[idx][level_indexes,:])
                        for idx in model.index]).T
    seasonal_data = array([dot(model.Z[idx][:,seasonal_indexes], model.a_hat[idx][seasonal_indexes,:])
                           for idx in model.index]).T
    ax = axs[0]
...
def plot_fig84(model, confidence=0.9):
    ...
    auxillary_residuals_u = model.u[nondiffuse_index] / \
                    ravel(sqrt(model.D[nondiffuse_index].to_list()))
